...
class CategoryEmbeddingRepository {
  ...
  Future<List<CategoryEmbedding>> getAll() async {
    ...
    final rows = await db.query('category_embeddings');

    return rows.map((row) => CategoryEmbedding.fromMap(row)).toList();
  }

  Future<CategoryEmbedding?> getByCategoryId(int categoryId) async {
    final db = await _appDatabase.database;
  ...
  Future<void> save(CategoryEmbedding categoryEmbedding) async {
    final db = await _appDatabase.database;
  ...
  Future<void> deleteAll() async {
    ...
    await db.delete('category_embeddings');
  }
